...
with open('Percentage_internet_users_per_country.csv', 'rb') as csvfile:
    rawdata = csv.reader(csvfile, delimiter=',', quotechar='|')

    # outputs txt with formatted array of arrays
    with open('data.txt','wb') as output_file:
        output_file.write("data: {")
        for row in rawdata:
            for i in range(0, len(country_codes)):
                if (row[0] == country_codes[i][2]):
                    row[0] = country_codes[i][1]
                    output_file.write("'" + country_codes[i][1] + "':{fillKey:'" + fillkey[int(float(row[1]) / 0.1)] +"', percentage:'" + str(float(row[1])*100) + "%'}, ")
